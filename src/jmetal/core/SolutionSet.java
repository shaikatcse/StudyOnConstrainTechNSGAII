...
public class SolutionSet implements Serializable {
	...
	/** 
   * get feasible slutions
   */
  
  public SolutionSet getFeasibleSolution() {
	  ArrayList<Solution> sArr = new ArrayList<Solution>();
	  if (size()>0) {
		  for (Solution aSolutionsList_ : solutionsList_) {
	          if (aSolutionsList_.getOverallConstraintViolation() >= 0.0) {
	        	  sArr.add(aSolutionsList_);	
	          }
		  }
	  }
	  SolutionSet temp = new SolutionSet(sArr.size());
	  for(Solution s:sArr) {
		  temp.add(s);
	  }
	  return temp;	 
